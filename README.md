...
Readme API keys, Clojars API tokens, OpenAI keys, Anthropic keys, npm access tokens, SendGrid keys, Twilio API keys, Azure
...
2. Runs the same two-tier ruleset `secret-scan-action` uses:
   - **Pattern rules (high confidence)** — distinctive formats that are
     ...
     (`rdme_...`, 70-char lowercase alphanumeric), and Clojars API tokens
     (`CLOJARS_...`, case-insensitive, 60-char alphanumeric) are high
     confidence — a fixed prefix and exact length, same as the other
